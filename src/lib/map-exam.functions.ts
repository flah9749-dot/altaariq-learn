...
${useGrid ? `⚠️ الصورة الثانية عليها شبكة مرجعية دقيقة ${cols}×${rows}: الأعمدة A..${String.fromCharCode(64 + Math.min(cols, 26))} (يسار→يمين)، الصفوف 1..${rows} (أعلى→أسفل). كل خانة مكتوب في زاويتها العليا اليسرى اسمها (مثل "H14") بلون أحمر.
- لكل نقطة انظر أولًا في الخريطة الأصلية لتحديد مركز المعلم الجغرافي بدقة (وسط الجزيرة، قمة الجبل، منتصف البحيرة/المحيط، مصب النهر).
- ثم انتقل للصورة المشبكة، وحدد بصريًا الخانة التي يقع مركز المعلم داخلها فعليًا. اقرأ اسم الخانة المطبوع داخلها فقط، ولا تخمّن الخانة المجاورة.
- أعِد أيضًا "cell_x" و"cell_y" كأرقام من 0 إلى 1 تمثل موضع مركز المعلم داخل هذه الخانة (0=يسار/أعلى، 0.5=منتصف، 1=يمين/أسفل).
- ضع الرقم على مركز المعلم نفسه، وليس على النص المكتوب أو السهم أو مفتاح الخريطة.
- إذا كان المعلم كبيرًا (قارة/محيط) اختر مركزه البصري لا حافته.
- إذا لم تكن متأكدًا 100٪ من الخانة الصحيحة، احذف النقطة تمامًا بدلاً من التخمين.
