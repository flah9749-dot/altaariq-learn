...
import { Download, FolderOpen, Loader2, Search, Trash2, Upload } from "lucide-react";
  ...
  const handleUpload = async (files: FileList | null) => {
    ...
    try {
      for (const file of Array.from(files)) {
        const key = `${user.id}/${Date.now()}-${crypto.randomUUID()}-${file.name}`;
        ...
        await upFn({ data: {
          ...
          target_class_id: null, target_group_id: null,
        } });
      }
          ...
          {isLoading ? <div className="space-y-2">{Array.from({length:5}).map((_,i)=><Skeleton key={i} className="h-12"/>)}</div>
            ...
            : <Table>
                <TableHeader><TableRow>
                  <TableHead>الاسم</TableHead><TableHead>الحجم</TableHead>
